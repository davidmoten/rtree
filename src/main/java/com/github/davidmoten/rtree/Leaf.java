...
final class Leaf<T> implements Node<T> {
    ...
    public Node<T> add(Entry<T> entry, ImmutableStack<NonLeaf<T>> stack) {
        ...
        if (newChildren.size() <= context.maxChildren()) {
            ...
            final ListPair<Entry<T>> pair = context.splitter().split(newChildren,
                    context.minChildren());
    ...
    private static <R> Optional<Node<R>> replace(Node<R> node, Node<R> replacement,
            ImmutableStack<NonLeaf<R>> stack, Context context) {
        return replace(node, Collections.singletonList(replacement), stack, context);
    }
    ...
    private static <R> Optional<Node<R>> replaceWhenStackNonEmpty(Node<R> node,
        ...
        final List<? extends Node<R>> newChildren = Util.replace(n.children(), node, replacements);
        if (newChildren.size() == 0) {
            return replace(n, Collections.<Node<R>> emptyList(), stack.pop(), context);
            ...
            final ListPair<? extends Node<R>> pair = context.splitter().split(newChildren,
                    context.minChildren());
            final NonLeaf<R> node1 = new NonLeaf<R>(pair.group1().list(), context);
            final NonLeaf<R> node2 = new NonLeaf<R>(pair.group2().list(), context);
            @SuppressWarnings("unchecked")
