...
public class SplitterTopological implements Splitter {
	...
	public SplitterTopological(ListPairMetric metric) {
		this.comparator = Comparators.compose(overlapListPair(metric),
	...
	public <T extends HasGeometry> ListPair<T> split(List<T> items, int minSize) {
		...
		map.put(SortType.X_LOWER,
				getPairs(minSize, sort(items, INCREASING_X_LOWER)));
		map.put(SortType.X_UPPER,
				getPairs(minSize, sort(items, INCREASING_X_UPPER)));
		map.put(SortType.Y_LOWER,
				getPairs(minSize, sort(items, INCREASING_Y_LOWER)));
		map.put(SortType.Y_UPPER,
				getPairs(minSize, sort(items, INCREASING_Y_UPPER)));
		...
		SortType leastMarginSumSortType = Collections.min(sortTypes,
				marginSumComparator(map));
	...
	private static <T extends HasGeometry> List<ListPair<T>> getPairs(
			int minSize, List<T> list) {
