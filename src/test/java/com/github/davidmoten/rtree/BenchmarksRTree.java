...
public class BenchmarksRTree {
	...
	private final RTree<Object> smallStarTreeM4 = RTree.maxChildren(4).star()
			.create().add(some);
	...
	private static List<Entry<Object>> entries1000() {
		...
		try {
			while ((line = br.readLine()) != null) {
				...
				list.add(Entry.entry(new Object(),
						Geometries.rectangle(x, y, x + 1, y + 1)));
			}
			br.close();
		} catch (IOException e) {
	...
	public static void main(String[] args) {
		...
		System.out.println(tree
				.search(Geometries.rectangle(500, 500, 630, 630)).count()
				.toBlocking().single());
